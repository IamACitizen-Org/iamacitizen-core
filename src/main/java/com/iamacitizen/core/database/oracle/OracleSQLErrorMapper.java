...
public class OracleSQLErrorMapper extends AbstractSQLErrorMapper {

	public OracleSQLErrorMapper(AbstractDatabase database) {
		super(database);
	...
	private String parentKeyViolation(String message, String initialMessage) {
		...
		if (c != null) {
			try {
				String alias = "";
				AnnotationDescriptor descriptor = AnnotationDescriptorFinder.findDescriptor(c.getRef().getTable());
